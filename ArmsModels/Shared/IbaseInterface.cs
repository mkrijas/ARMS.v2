...
namespace ArmsServices.DataServices
{
    public interface IbaseInterface<T> where T : class
    {
        ...
        IEnumerable<T> SelectByApproved(int? BranchID, int? NumberOfRecords, string searchTerm);
        IEnumerable<T> SelectByUnapproved(int? BranchID, int? NumberOfRecords, string searchTerm);
        T Update(T model);
    }
}
