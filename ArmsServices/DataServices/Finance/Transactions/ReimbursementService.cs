...
namespace DAL.DataServices.Finance
{
    public class ReimbursementService : IReimbursementService
    {
        ...
        public PagedResult<InterBranchReimbursementModel> SelectAll(int? BranchID, int page, int pageSize, string search, bool _IsApproved)
        {
            ...
            List<InterBranchReimbursementModel> list = [];
            int? countOf = 0;
            foreach (IDataRecord dr in Iservice.GetDataReader("[usp.Finance.Transactions.Reimbursement.Select]", parameters))
            {
            ...
            return new PagedResult<InterBranchReimbursementModel>
            {
                Items = list,
